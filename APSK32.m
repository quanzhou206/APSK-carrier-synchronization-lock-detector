...
ELockDetect = zeros(1,length(snr)); % 锁定检测量均值
SigLockDetect = zeros(1,length(snr)); % 锁定检测量均值
ELockDetect_w = zeros(1,length(snr)); % 锁定检测量均值
SigLockDetect_w = zeros(1,length(snr)); % 锁定检测量均值

Seg_th = [(sqrt(2)+3)/2,(5+3)/2];
weight_matrix = [2/36,9/36,25/36];% 2个圈权重
...
%         dataclass1 = get_class(abs(dataMod(i)),Seg_th);
        ...
        elseif dataclass == 2
            ...
            xMN_w(dataclass) = xMN_w(dataclass) + ...
                            real(rxFiltSignal(i).^16)./abs(rxFiltSignal(i)).^16; 
        end
        xMN = xMN + (real(rxFiltSignal(i).^16)./abs(rxFiltSignal(i)).^16); 
        xMN_w_num(dataclass) = xMN_w_num(dataclass) + 1;
    end
    dataDetect_c1(epoch,index) = abs(xMN)/N_sym;
    dataDetect_c2(epoch,index) = (abs(xMN_w(3))./xMN_w_num(3));
...
y = zeros(3,length(EbNo));
y_2 = zeros(3,length(EbNo));
for i=1:3
    EsN0_class = snr +  10*log10(SigAm(i)/averAm);
    ESN0 = 10.^(EsN0_class./10);
    if i==1
        y(i,:) = exp(-4^2./(4*ESN0));
        y_2(i,:) = ((0.5 + exp(-4^2./ESN0)/2) - y(i,:).^2)/(N_sym/8);
    elseif i==2
        y(i,:) = exp(-12^2./(4*ESN0));
        y_2(i,:) = ((0.5 + exp(-12^2./ESN0)/2) - y(i,:).^2)/(3*N_sym/8);
    else
        y(i,:) = exp(-16^2./(4*ESN0));
        y_2(i,:) = ((0.5 + exp(-16^2./ESN0)/2) - y(i,:).^2)/(N_sym/2);        
    end
end
y_aver_MN = exp(-16^2./(4*(10.^(snr./10))));
Sig_aver_MN = (0.5 + exp(-16^2./(10.^(snr./10)))/2) - y_aver_MN.^2;
